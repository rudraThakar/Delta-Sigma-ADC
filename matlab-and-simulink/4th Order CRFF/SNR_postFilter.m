...
Fs  = 2e3;       % Sampling frequency for all cases
...
if evalin('base','exist(''post_dec'',''var'')')
    tmp = evalin('base','post_dec');
...
if ~isempty(yCT)
    ...
    warning('CT output not found in workspace. Ensure To Workspace var name = ''post_dec'' or logsout contains CT signal.');
    results.SNR_CT = NaN;
    results.ENOB_CT = NaN;
end
